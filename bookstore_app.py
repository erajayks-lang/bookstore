...
def initialize_data():
    ...
    # Initialize books - Load 200 books from pre-generated file
    books = load_json(BOOKS_FILE, [])
    if not books:
        # Try to load from books_200.json if it exists
        if os.path.exists('books_200.json'):
            try:
                with open('books_200.json', 'r') as f:
                    books = json.load(f)
                save_json(BOOKS_FILE, books)
            except Exception as e:
                # Fallback to minimal books
                books = [
                    {'id': 1, 'title': 'The Great Gatsby', 'author': 'F. Scott Fitzgerald', 'price': 12.99, 'category': 'Fiction', 'description': 'Classic American novel.', 'stock': 50, 'image': '📕'},
                    {'id': 2, 'title': '1984', 'author': 'George Orwell', 'price': 13.99, 'category': 'Science Fiction', 'description': 'Dystopian novel.', 'stock': 60, 'image': '📘'},
                    {'id': 3, 'title': 'The Hobbit', 'author': 'J.R.R. Tolkien', 'price': 15.99, 'category': 'Fantasy', 'description': 'Fantasy adventure.', 'stock': 55, 'image': '📙'},
                ]
                save_json(BOOKS_FILE, books)
        else:
            # Fallback books if books_200.json doesn't exist
            books = [
                {'id': 1, 'title': 'The Great Gatsby', 'author': 'F. Scott Fitzgerald', 'price': 12.99, 'category': 'Fiction', 'description': 'Classic American novel.', 'stock': 50, 'image': '📕'},
                {'id': 2, 'title': '1984', 'author': 'George Orwell', 'price': 13.99, 'category': 'Science Fiction', 'description': 'Dystopian novel.', 'stock': 60, 'image': '📘'},
                {'id': 3, 'title': 'The Hobbit', 'author': 'J.R.R. Tolkien', 'price': 15.99, 'category': 'Fantasy', 'description': 'Fantasy adventure.', 'stock': 55, 'image': '📙'},
